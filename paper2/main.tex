...
\subtitle{Version 2 (September 2026): adds exact $k$-NN, eigen-curve
cascade-width selection, GIST1M / DBpedia-OpenAI-1M / DEEP10M at full
scale, measured out-of-core cold reads, and an ADSampling baseline.
Version 1 (August 2026): doi:10.5281/zenodo.22199956.}
...
\textbf{Tier 1 --- eigenvalue decay, which now also \emph{picks} the
widths.} Fit PCA on a sample; the cumulative explained variance at each
cascade width is the expected tightness of that PCA level. Version 1 of
this work (and Paper~I) used a fixed ladder $(8,16,32,64)$---which, we
...
Fit PCA (as in Paper~I; covariance accumulated in chunks, so no full
...
PCA-only~\cite{paper1} & 12.60 & 0 & \textbf{10.2$\times$} \\
...
PCA-only~\cite{paper1} & yes & 61.1 & 12.6 & 32\,MB \,(16$\times$) \\
...
Table~\ref{tab:datasets} answers the question version~1 left open: does
...
versus 62.2\,ms). Version 2 extends the check to every new dataset: GIST1M
...
Version 1 argued the out-of-core case from a cost model; version 2 measures
it (Table~\ref{tab:ooc}). The protocol: all reads use \texttt{O\_DIRECT}
...
Version 2 closes the loop the first version opened: the self-configuration
now includes the cascade widths themselves (without which 1536-dimensional
embeddings masquerade as unprunable), the diagnostic's sample verdicts are
confirmed at $10^6$--$10^7$ scale on four real datasets, exact $k$-NN is
implemented with its honest breaking point stated, and the out-of-core
argument has become a measurement: exact cold-read queries $7$--$17\times$
faster than a disk scan at $40$--$730\times$ fewer bytes, against a
state-of-the-art statistical pruner that structurally cannot leave RAM.
The recurring lesson is that every constant we had silently tuned to one
...
\bibitem{adsampling}
J. Gao and C. Long.
\newblock High-dimensional approximate nearest neighbor search: with
  reliable and efficient distance comparison operations.
\newblock {\em Proc.\ ACM Manag.\ Data (SIGMOD)}, 1(2), 2023.

\bibitem{pdx}
L. Kuffo, E. Krippner, and P. Boncz.
\newblock PDX: A data layout for vector similarity search.
\newblock {\em Proc.\ ACM Manag.\ Data (SIGMOD)}, 3(3), 2025.
